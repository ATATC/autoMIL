...
### Round 56-58 (2026-03-16)

- **Coord PE scale 0.2**: composite 0.840 (-0.012). Too strong, PE overwhelms features.
- **Coord PE scale 0.05**: composite 0.851 (-0.0003). Almost identical to 0.1, very robust.
- **Instance masking (ACMIL-inspired)**: composite 0.565 (-0.286). Catastrophic, hard masking always fails.
- **MaxSoft pooling**: composite 0.816 (-0.036). Straight-through estimator doesn't work well with CLAM.
- **SupCon loss (weight=0.2)**: composite 0.806 (-0.046). Contrastive learning on 2D logits is too weak.
- **Bi-level LR (attn 2x)**: composite 0.767 (-0.084). Uniform LR is already optimal.
- **Learned attention temperature**: composite 0.844 (-0.008). Temperature hurts, fixed softmax is better.
- **Coord PE + R-Drop alpha=0.5**: composite 0.851 (-0.0006). Near-identical to alpha=1.0 with coord PE.
- **Late fusion hoptimus1+uni_v2**: CRASHED (model cleanup before probs extraction).

Key insight: The coord PE (scale=0.1) remains the only improvement since step LR. The model is extremely robust to all architectural changes, suggesting we may be near the data-limited ceiling for this task at ~0.851 composite.
