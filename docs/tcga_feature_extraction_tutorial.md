...
**Reference:** Leo's completed TCGA-LUAD setup serves as the worked example. Look at:
- `benchmarks/datasets/tcga_luad.yaml` — config reference
- The TCGA-LUAD output directory — expected file structure
...
# Count .pt files per model
for model in virchow2 hoptimus1 uni_v2; do
    echo "$model: $(ls $DATASET_ROOT/trident_output/$model/pt_files/*.pt 2>/dev/null | wc -l)"
done
...
└── trident_output/
    ...
    ├── skipped_slides.txt                # Corrupted slides (if any)
    ├── 20x_224px_0px_overlap/            # Patch coordinates
    │   ├── TCGA-XX-XXXX-01Z-00-DX1.csv
    │   └── ...
    ├── virchow2/
    │   └── pt_files/                     # 2560-dim feature tensors
    │       ├── TCGA-XX-XXXX-01Z-00-DX1.pt
    │       └── ...
    ├── hoptimus1/
    │   └── pt_files/                     # 1536-dim feature tensors
    │       └── ...
    └── uni_v2/
        └── pt_files/                     # 1536-dim feature tensors
            └── ...
...
Each `.pt` file contains a `torch.Tensor` of shape `(num_patches, embedding_dim)`:
- virchow2: `(N, 2560)`
- hoptimus1: `(N, 1536)`
- uni_v2: `(N, 1536)`
...
| Count results | `ls datasets/{DATASET}/trident_output/virchow2/pt_files/*.pt \| wc -l` |
